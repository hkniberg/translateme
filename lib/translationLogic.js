 ...
 */
export function getAllLanguageDatasForProject({owner, repo, baseLanguageFilePath, gitHubAccessToken}) {
  ...
  if (baseLanguageFilePath) {
    ...
    for (let i = 0; i < plugins.length; ++i) {
      ...
      for (let j = 0; j < dirs.length; ++j) {
        ...
        languageDatas = tryToGetLanguageDatasForDirAndPlugin({owner, repo, dir, plugin, gitHubAccessToken})
 ...
 */
function tryToGetLanguageDatasForDirAndPlugin({owner, repo, dir, plugin, gitHubAccessToken}) {
  maybeLog("tryToGetLanguageDatasForDirAndPlugin", owner, repo, dir)
  check(dir, String)
  check(plugin, Object)
  ...
  if (gitHubFiles) {
    ...
    gitHubFiles.forEach((gitHubFile) => {
      ...
      let languageData = tryToCreateLanguageData({owner, repo, plugin, gitHubFile, fileContents, gitHubAccessToken})
      if (languageData) {
 ...
 */
function tryToCreateLanguageData({owner, repo, plugin, gitHubFile, fileContents, gitHubAccessToken}) {
  ...
  try {
    ...
    maybeLogError("(ignoring) Plugin " + plugin.getName() + " couldn't get language of file " + gitHubFile.name, err)
    return null
  }
  ...
  try {
    ...
    maybeLogError("(ignoring) Plugin " + plugin.getName() + " couldn't understand the contents of " + gitHubFile.name, err)
    return null
  }
...
export function _getReviewData(fromOwner, toOwner, repo, fromPath, toPath, fileFormat, gitHubAccessToken) {
  console.log(1)
  try {
    const fromLanguageData = getLanguageDataFromPath(fromOwner, repo, fromPath, fileFormat, gitHubAccessToken)
    const toLanguageData = getLanguageDataFromPath(toOwner, repo, toPath, fileFormat, gitHubAccessToken)
    console.log(2)
    return {
    ...
    console.log("getLanguageDataFromSession threw error", err)
    throw err
  }
...
export function _getLanguageInfo(owner, repo, path, gitHubAccessToken) {
