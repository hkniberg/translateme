...
export function submitTranslation({owner, repo, fileFormat, fromLanguageFile, toLanguageCode, texts, commitComment, gitHubAccessToken}) {
  console.log("submitTranslation", owner, repo, fileFormat, fromLanguageFile, toLanguageCode, "" + texts.length + " texts", commitComment, gitHubAccessToken)
  ...
  if (isForkNeeded) {
    ...
    //If fork already existed, this does nothing.
    ...
    //Make sure the fork is up-to-date (in case the fork already existed and was out of date)
    syncFork({currentGitUser, owner, repo, gitHubAccessToken})
    result.log.push("Synked the fork to match the latest contents of " + owner + "/" + repo)
...
function tryToGetLanguageDatasForDirAndPlugin({owner, repo, dir, plugin, languageCodes, gitHubAccessToken}) {
  ...
  if (gitHubFiles) {
    ...
    gitHubFiles.forEach((gitHubFile) => {
      ...
      if (!languageCode) {
        maybeLog("Ignoring " + gitHubFile.path + ", because I don't know which language code it represents")
      } else if (!languageCodes || languageCodes.includes(languageCode)) {
        const fileContents = getHttpFileContents(gitHubFile.download_url)
        let languageData = tryToCreateLanguageDataFromFileContents({owner, repo, plugin, gitHubFile, fileContents, gitHubAccessToken})
        if (languageData) {
          maybeLog("Found a languageData with " + languageData.textCount + " texts")
...
function createLanguageDataFromFileContents({owner, repo, plugin, gitHubFile, fileContents, gitHubAccessToken}) {
  ...
  try {
    ...
    throw new Meteor.Error("cantParseFileName", "Plugin " + plugin.getName() + " couldn't figure out the language from filename " + gitHubFile.name + ": " + err, err)
  }
  if (!languageCode) {
    throw new Meteor.Error("cantParseFileName", "Plugin " + plugin.getName() + " couldn't figure out the language from filename " + gitHubFile.name)
  }
  ...
  try {
    ...
    throw new Meteor.Error("cantParseFileContents", "Plugin " + plugin.getName() + " couldn't understand the contents of " + gitHubFile.name + ": " + err, err)
  }
...
function createPullRequest({currentGitUser, owner, repo, comment, gitHubAccessToken}) {
  const params = {
    title: comment,
    base: "master", //where to send the pull request
    head: currentGitUser + ":master", //where to pull from
    body: comment
  }
...
/*
  Makes sure the current user's fork of the given repo is up-to-date.

  Using this trick: https://stackoverflow.com/questions/26846667/how-to-update-a-fork-from-its-original-via-the-github-api
  Using Git data api: This will usually be the best option, if you don't change your fork's master.

 */
function syncFork({currentGitUser, owner, repo, gitHubAccessToken}) {

  //Get upstream ref /repos/upstream/repo/git/refs/heads/master, and get the hash from it
  const result = callGitHub({owner, repo, command: "git/refs/heads/master", gitHubAccessToken, method: "GET"})
  const upstreamRefHash = result.object.sha

  //Update the fork PATCH /repos/my/repo/git/refs/heads/master with the same hash.
  const params = {
    sha: upstreamRefHash,
    force: true
  }
  callGitHub({owner: currentGitUser, repo, command: "git/refs/heads/master", gitHubAccessToken, method: "PATCH", params})
}
