...
function getLanguageInfos(owner, repo, baseLanguagePath, gitHubAccessToken) {
  ...
  if (baseLanguagePath) {
    //OK, were given a specific file, so we'll only search in that dir
    ...
    localeFiles.forEach((localeFile) => {
      //download it
      const contents = getHttpFileContents(localeFile.download_url)
      console.log("contents", contents)
      const localeFilePath = localeFile.name
      console.log("localeFile.name", localeFile.name)
      //find the first plugin that can parse it
      for (let i = 0; i < plugins.length; ++i) {
        const plugin = plugins[i]
        try {
          const languageCode = plugin.getLanguageOfFile(localeFilePath)
          if (languageCode && plugin.convertFileContentsToLanguageTexts(localeFile.name, contents)) {
            ...
            break //stop looping through plugins and go to the next file instead
          }
        } catch (err) {
          //ignore it
        }
      ...
      for (let j = 0; j < localeDirPaths.length; ++j) {
        ...
        try {
          ...
          localeFiles.forEach((localeFile) => {
            ...
            const languageCode = plugin.getLanguageOfFile(localeFilePath)
            if (languageCode) {
              //Yay, found a locale file! Let's see if it is parseable.
              const languageInfo = {
                languageCode: languageCode,
                languageName: getLanguageName(languageCode),
                path: localeFile.path,
