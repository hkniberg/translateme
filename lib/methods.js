...
function submitTranslation(owner, repo, fromLanguageInfo, toLanguageCode, texts, commitComment, gitHubAccessToken) {
  console.log("submitTranslation", owner, repo, fromLanguageInfo, toLanguageCode, texts, commitComment, gitHubAccessToken)
...
function createPullRequest(currentGitUser, owner, repo, comment, gitHubAccessToken) {
  ...
  const result = callGitHub(owner, repo, "pulls", gitHubAccessToken, "POST", params)
...
function tryToCreateLanguageInfo(owner, repo, plugin, gitHubFile, fileContents, gitHubAccessToken) {
  ...
  let textCount = tryToCountLanguageTexts(gitHubFile.name, fileContents, plugin)
  if (textCount == null) {
    return null
  }
  ...
  console.log("Created languageInfo", languageInfo)
  return languageInfo
}

/**
  Counts the number of texts in the given language file,
  or returns null if it couldn't be parsed
*/
function tryToCountLanguageTexts(fileName, fileContents, plugin) {
  try {
    const languageTexts = plugin.convertFileContentsToLanguageTexts(fileName, fileContents)
    console.log("property names", Object.getOwnPropertyNames(languageTexts))
    return Object.getOwnPropertyNames(languageTexts).length
  } catch (err) {
    if (logAllStackTraces) {
