...
Meteor.methods({
  /*
    Returns an array of LanguageInfos for each language found in the repo,
    or null if I don't recognize the format.
   */
   ...
   */
  getLanguageDatas(owner, repo, fromLanguageCode, toLanguageCode, gitHubAccessToken) {
    if (Meteor.isServer) {
      return getLanguageDatas(owner, repo, fromLanguageCode, toLanguageCode, gitHubAccessToken)
    }
  },
...
function submitTranslation(owner, repo, fromLanguageCode, toLanguageCode, fullTranslation, comment, gitHubAccessToken) {
  console.log("submitTranslation", owner, repo, fromLanguageCode, toLanguageCode, fullTranslation, comment, gitHubAccessToken)
...
/*
Returns an array of all languageInfos in the given rep
 */
...
function getLanguageTexts(owner, repo, languageCode, gitHubAccessToken) {
  const languages = getLanguageInfos(owner, repo, gitHubAccessToken)
...
function getLanguageDatas(owner, repo, fromLanguageCode, toLanguageCode, gitHubAccessToken) {
  console.log("getLanguageDatas", owner, repo, fromLanguageCode, toLanguageCode, gitHubAccessToken)

  const languageInfos = getLanguageInfos(owner, repo, gitHubAccessToken)
  
  const fromLanguageInfo = languageInfos.find((languageInfo) => {
    return languageInfo.languageCode == fromLanguageCode
  })
  let toLanguage
  let toLanguageInfo = languageInfos.find((languageInfo) => {
    return languageInfo.languageCode == toLanguageCode
  })
  if (toLanguageInfo) {
    //Language already existed. Good. Load the data.
    toLanguage = getLanguageData(toLanguageInfo)
  } else {
    //Language didn't already exist. Create an empty languageData.
    toLanguage = {
...
function getLanguageData(languageInfo) {
  console.assert(languageInfo, "Missing languageInfo")
  const languageData = {
  ...
  languageData.texts = getHttpFileAsJson(languageInfo.downloadUrl)
  return languageData
}
