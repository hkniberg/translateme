...
function submitTranslation(owner, repo, fromLanguageInfo, toLanguageCode, texts, commitComment, gitHubAccessToken) {
  ...
  if (isForkNeeded) {
    ...
    result.commitUrl = commitFile(currentGitUser, repo, translatedFilePath, fileContent, commitComment, gitHubAccessToken)
    ...
    const commitSha = commitFile(owner, repo, translatedFilePath, fileContent, commitComment, gitHubAccessToken)
  ...
  for (let i = 0; i < plugins.length; ++i) {
    ...
    for (let j = 0; j < localeDirPaths.length; ++j) {
      ...
      try {
        ...
        console.log("Caught error, will ignore: " + err)
        //ignore
      }
