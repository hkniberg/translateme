 ...
 */
function submitTranslation(owner, repo, fromLanguageCode, toLanguageCode, texts, comment, gitHubAccessToken) {
  console.log("submitTranslation", owner, repo, fromLanguageCode, toLanguageCode, texts, comment, gitHubAccessToken)
  ...
  const fullTranslationAsString = JSON.stringify(texts)
...
function getLanguageDatas(owner, repo, fromLanguageInfo, toLanguageCode, gitHubAccessToken) {
  ...
  return {
    fromLanguage: getLanguageData(projectType, fromLanguageInfo),
    toLanguage: toLanguageData
  }
}

function getLanguageData(projectType, languageInfo) {
  assert(projectType, "Missing projectType")
  assert(languageInfo, "Missing languageInfo")

  const plugin = getPluginByName(projectType)
  assert(plugin, "No plugin named " + projectType)
  ...
  const fileContents = getHttpFileContents(languageInfo.downloadUrl)
  languageData.texts = plugin.convertFileContentsToLanguageTexts(fileContents)
